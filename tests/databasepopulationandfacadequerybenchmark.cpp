...
class DatabasePopulationAndFacadeQueryBenchmark : public QObject
{
    ...
    void populateDatabase(int count)
    {
        ...
        auto domainTypeAdaptorFactory = QSharedPointer<TestEventAdaptorFactory>::create();
        {
            ...
            for (int i = 0; i < count; i++) {
                ...
                const auto key = QString::number(i).toLatin1();
                db.write(key, buffer);
    ...
    void testLoad(int count)
    {
        ...
        //This shouldn't include the attachment (but currently does)
        QEXPECT_FAIL("", "We're loading the attachment", Continue);
        QVERIFY(rssGrowthPerEntity < 2000);
